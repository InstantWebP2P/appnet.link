// AppNet.Link name-client implementation
// Copyright (c) 2012-present Tom Zhou<appnet.link@gmail.com>

'use strict';
var debug = require('debug')('appnet.link');
...
nmCln.prototype._LSM_connectTurnAgent = function(fn, tmo){
    ...
    if (self.turnSrvs && self.turnSrvs.length) {
        ...
        self.turnagentConn = new WebSocket(wsproto+self.turnSrvs[0].ip+':'+self.turnSrvs[0].agent+SEP.SEP_CTRLPATH_AS,
                                           {
                                               ...
                                               hole: { addr: self.ipaddr, port: self.port },
                                               localAddress: self.ipaddr, localPort: self.port,
        ...
        self.turnagentConn.once('open', function(){            
            ...
            try {
                // !!!Prefer json format
                ///self.turnagentConn.send(MSGPACK.encode(tom), function(err){
                self.turnagentConn.send(JSON.stringify(tom), function(err){    
        ...
        self.turnagentConn.on('message', function(message){
...
nmCln.prototype._LSM_setupSdpSession = function() {
        ...
        conn = new WebSocket(wsproto+self.srvs[0].ip+':'+self.srvs[0].port+SEP.SEP_CTRLPATH_NS,
                             {
                                 ...
                                 hole: { addr: self.ipaddr },
                                 localAddress: self.ipaddr,
        ...
        for (var i = 1; i < self.srvs.length; i ++) {
            ...
            connalt.once('close', function(){
                ...
                if (self.state === 'ready') {
                    ...
                    if (!nmconlive) {
                        debug('alternate client reconnect triggered');

                        self.state = 'reconnecting';
                        self.emit('reconnect', 'name-server alternate client close');
                    } else {
                        debug('alternate client already recovered');
                    }
                }
            });
            connalt.on('error', function(err){
                console.error('name-server alternate client error '+err);
            });
        }
    });
    ...
    conn.once('close', function(){                        
        debug('name-server primary client close');
        ...
        if (self.state === 'ready') {
            ...
            if (!nmconlive) {
                debug('primary client reconnect triggered');
                
                self.state = 'reconnecting';
                self.emit('reconnect', 'name-server primary client close');
            } else {
                debug('primary client already recovered');
            }
        }
    });
            ...
            if (!err) {
                ...
                self.updateNatype(function(err){
                    if (!err) {
                        ...
                        if (self.reconnect == 0) {
                            // first connect
                            console.log('AppNet.link connect ready');
                            ...
                            // reconnect
                            console.log('AppNet.link reconnect ready '+self.reconnect+' times');
                        ...
                        console.error(err+',setup NAT report session failed');
                        self.state = 'error';
                        self.emit('error', err+',setup NAT report session failed');
                    }
                });
            } else {
                console.error(err+',setup SDP session failed');
                self.state = 'error';
...
nmCln.prototype._LSM_setupBusinessServer = function(){
    ...
    if (self.conmode === SEP.SEP_MODE_CS) {
        ...
        var bsrv_lisn_cb = function() {
            debug('Business server started, Go ...');
            self.emit('go');
        };

        if (self.secmode) // https options: key/cert/ca/...
            self.bsrv.srv = httpps.createServer(self.srvsslcerts).listen(self.port, self.ipaddr, self.backlog, bsrv_lisn_cb);
        else
            self.bsrv.srv = httpp.createServer().listen(self.port, self.ipaddr, self.backlog, bsrv_lisn_cb);
        ...
        self.bsrv.srv.on('connection', function(socket){
            if (self.checkAcl(socket.remoteAddress, socket.remotePort)) {
                debug('Pass ACL check on '+socket.remoteAddress+':'+socket.remotePort);
            } else {
                // TBD... block incoming connection by FW
                console.error('!!! DDOS attack from '+socket.remoteAddress+':'+socket.remotePort+ ', close it');
        ...
        self.bsrv.hpsrv.on('connection', function(client) {    
            console.log('new hole connection: ' +
            ...
            try {
                ...
                // !!!Prefer json format
                ///client.send(MSGPACK.encode(coninfo), function(err){
                client.send(JSON.stringify(coninfo), function(err){    
                    if (err) {
                        console.error(err+',send peer info failed');
                    }
                });
                ...
                var clrtmo = null;
                if (self.keephole) {
                    debug('keep connection of hole-punch server');
                } else {
                    clrtmo = setTimeout(function(){
                        if (client && client.close) client.close();

                        debug('close connection of hole-punch server');
                    }, 620000);
                }

                client.on('error', function(err){
                    console.error(err+',hole-punch client failed');
                ...
                client.once('close', function(){
                    debug('hole-punch client closed');
                ...
                console.error(e+',send peer info failed immediately');
            }
        });
        ...
        self.heartBeating(function(err, yes){
            if (!err && yes) {
                ...
                if (self.heartbeatTimeout > 2) {
                    debug('Hear-beat check timeout '+self.heartbeatTimeout);
                    ...
                    // trigger reconnect event
                    if (self.state === 'ready') {
                        debug('hear-beat checking triggered');
...
nmCln.prototype.createConnection = function(to, fn){
    ...
    var opt = {
        httpp: true, 
        hole: {port: self.port, addr: self.ipaddr},
        localAddress: self.ipaddr, localPort: self.port,
    };
    var conn;
    var wsburl = '';
    ...
    if (to.sesn === SEP.SEP_SESN_STUN) {
        ...
        // check for IPv6
        if (6 === UDT.isIP(peer.ip)) {
            wsburl = wsproto + '[' + peer.ip + ']:' + peer.port + wstoken + SEP.SEP_CTRLPATH_BS;
        } else {
            wsburl = wsproto +       peer.ip +  ':' + peer.port + wstoken + SEP.SEP_CTRLPATH_BS;
        }
    } else {
        ...
        // check vURL mode
        if (peer.vmode === vURL.URL_MODE_HOST) {
            // vHOST mode
            wsburl = wsproto + peer.vhost + peer.ip + ':' + peer.port + wstoken + SEP.SEP_CTRLPATH_BS;
        } else {
            // vPATH mode

            // check for IPv6
            if (6 === UDT.isIP(peer.ip))
                wsburl = wsproto + '[' + peer.ip + ']:' + peer.port + wstoken + peer.vpath + SEP.SEP_CTRLPATH_BS;
            else
                wsburl = wsproto +       peer.ip +  ':' + peer.port + wstoken + peer.vpath + SEP.SEP_CTRLPATH_BS;
        }
    }
    conn = new WebSocket(wsburl, opt);
    console.log('connect to ' + wsburl);
    ...
    */
    ...
    conn.once('close', function(){
        ...
        debug('new peer connection closed to '+JSON.stringify(peer));
    });
};
...
nmCln.prototype.punchHole = function(to, fn){
    ...
    debug('punchHole, endinfo:'+JSON.stringify(endinfo));
 
    // Hole Punch algorithem
    // - 0: one side is directed(Not in nat/fw), punching hole only from another side to the directed host
    // - 1: both side are asymmetric, punching each other with a few pacekt
    // - 2: only one side are asymmetric, punching from asymmetric only with a lot packet
    // - 3: both side are symmetric, going to TURN session anyway

    debug('self.natype: '+self.natype+',peer.natype:'+peer.natype);
        
    // 0.
    // punch hole with at lease one directed host without NAT/FW
    if ((self.natype === 2) || (peer.natype === 2)) {
        fn('TBD...directed host');
        
        if ((self.natype != 2) || (self.natype === 2 && to.isInitiator)) {
            // 0.1
            //  Nat/fw or Initiator none-nat/fw side algorithem:
            // - allow incomming wss connection from peer for all ports
            // - waiting peer connection
            // - send peer's external ip/port info to peer by holepunch server
            // - re-allow incomming wss connection from peer for hole port only ???
            ...
            // 0.2
            // None-Initiator none-nat/fw side algorithem:
            // - allow incomming wss connection from peer for all ports
            // - waiting peer connection
            // - send peer's external ip/port info to peer by holepunch server
            // - re-allow incomming wss connection from peer for hole port only ???
    ...
    if (self.natype===0 && peer.natype===0) {
        ...
        }, 200); // punch hole about every 200ms
        ...
        setTimeout(function() {
            ...
            if (to.isInitiator) {
                ...
                // check for IPv6
                if (6 === UDT.isIP(peer.ip)) {
                    wshurl = wsproto+'['+peer.ip+']:'+peer.port+SEP.SEP_CTRLPATH_HS;
                } else { 
                    wshurl = wsproto+peer.ip+':'+peer.port+SEP.SEP_CTRLPATH_HS;
                }
                debug('ws hole url: '+wshurl);
                conn = new WebSocket(wshurl, opt);
                ...
                conn.once('message', function(message) {
                    debug('punch hole notification messaging in %s mode from %s',
                    (to.mode === SEP.SEP_MODE_PP) ? 'p2p' : 'c/s',
                    (to.isInitiator)? 'initiator' : 'peer');
                    
                    var coninfo = typeof message !== 'string' ? MSGPACK.decode(message) : JSON.parse(message);
                    ...
                    if (self.trvsStunCache && 
                        (self.trvsStunCache[peer.vhost] || 
                        ...
                        if (self.trvsStunCache[peer.vhost]) {
                            ...
                            debug(peer.vhost+': increase stun count to '+self.trvsStunCache[peer.vhost].stun_count);
                        ...
                        if (self.trvsStunCache[peer.vpath]) {
                            ...
                            debug(peer.vpath+': increase stun count to '+self.trvsStunCache[peer.vpath].stun_count);
                ...
                conn.on('error', function(err){
                    ...
                    console.error('punch hole error %s in %s mode from %s', err,
                    (to.mode === SEP.SEP_MODE_PP) ? 'p2p' : 'c/s',
                    (to.isInitiator)? 'initiator' : 'peer');
                });
           
                conn.once('close', function(){
                    debug('punch hole closed in %s mode from %s',
                                    (to.mode === SEP.SEP_MODE_PP) ? 'p2p' : 'c/s',
                                    (to.isInitiator)? 'initiator' : 'peer');
                    ...
                    if (self.trvsStunCache && 
                        ...
                        if (self.trvsStunCache[peer.vhost]) {
                            ...
                            debug(peer.vhost+': decrease stun count to '+self.trvsStunCache[peer.vhost].stun_count);
                            
                            if (self.trvsStunCache[peer.vhost].stun_count === 0) {
                                self.trvsStunCache[peer.vhost] = null;
                                
                                debug(peer.vhost+': clear stun session ');
                            }
                        }
                        ...
                        if (self.trvsStunCache[peer.vpath]) {
                            ...
                            debug(peer.vpath+': decrease stun count to '+self.trvsStunCache[peer.vpath].stun_count);

                            if (self.trvsStunCache[peer.vpath].stun_count === 0) {
                                self.trvsStunCache[peer.vpath] = null;
                                
                                debug(peer.vpath+': clear stun session');
                            }
                        }
                    } 
                });
    ...
    if (self.natype === 1 || peer.natype === 1) {
        ...
        if (self.natype === 1) {
            ...
            setTimeout(function(){
                ...
                debug('symmetric ws hole url: ' + wshurl);
                ...
                conn.once('message', function(message) {
                    debug('punch hole notification messaging in %s mode from %s @symmetric',
                    (to.mode === SEP.SEP_MODE_PP) ? 'p2p' : 'c/s',
                    (to.isInitiator)? 'initiator' : 'peer');
                    ...
                    // 2.1.4.2
                    // maintain or close hole-punch connection
                    // if hole is punched, close keep-hole connection after 6s
                    if (self.trvsStunCache[peer.vhost] || 
                        self.trvsStunCache[peer.vpath]) {
                        // check stun session count
                        if (self.trvsStunCache[peer.vhost]) {
                ...
                conn.once('close', function(){
                    debug('punch hole closed in %s mode from %s @symmetric',
                                    (to.mode === SEP.SEP_MODE_PP) ? 'p2p' : 'c/s',
                                    (to.isInitiator)? 'initiator' : 'peer');
            ...
            hole = UDT.createHole(endinfo);
            endinfo.hole = endinfo.hole || {};
            endinfo.port = 49152;
            endinfo.hole.holeRange = {from: 0, to: 65535 - 49152};
            hole.punchhole(endinfo); // punch hole once
        ...
        // Don't support it
        fn('NAT Not support for hole punch');
    }
};
...
nmCln.prototype.offerSdp = function(fn, tmo){
    ...
    debug('pre-event:'+'clntsdpanswer'+self.clntsdpanswerCbCnt);
    self.once('clntsdpanswer'+self.clntsdpanswerCbCnt, function(sdp){
        debug('after-event:'+'clntsdpanswer'+self.clntsdpanswerCbCnt);
    ...
    for (var k in self.conn) {
        ...
        opc_msg.offer.srvpublicIP   = (self.conn[k].socket && self.conn[k].socket.remoteAddress) || self.conn[k].to.ip;
        opc_msg.offer.srvpublicPort = (self.conn[k].socket && self.conn[k].socket.remotePort)    || self.conn[k].to.port;
        ...
        try {
            // !!!Prefer json format
            ///self.conn[k].socket.send(MSGPACK.encode(opc_msg), function(err){
            self.conn[k].socket.send(JSON.stringify(opc_msg), function(err){
                if (err) console.log(err+',send sdp info failed');
            });
        } catch (e) {
            console.error(e+',send sdp info failed immediately');
        }
    }
};
...
nmCln.prototype.sendOpcMsg = function(opc_msg, fn){
    ...
    if ((self.conn[con0] && self.conn[con0].socket) && 
        ...
        try {
            ...
            // !!!Prefer json format
            ///self.conn[con0].socket.send(MSGPACK.encode(opc_msg), function(err){
            self.conn[con0].socket.send(JSON.stringify(opc_msg), function(err) {    
                if (err) {
            ...
            console.error(e+',sendOpcMsg failed immediately');
            if (fn) fn(e+',sendOpcMsg failed immediately');
        }
    } else {
        // Algorithem:
        // 1. waiting for on fly reconnecting within 6s
        // 2. delayed sendOpcMsg execution once reready

        var dt = setTimeout(function(){
            console.error('sendOpcMsg reconnect retry broken after 6s');
            if (fn) fn('sendOpcMsg reconnect retry broken after 6s');
        }, 6000); // 6s timeout

        self.once('reready', function(){
            clearTimeout(dt);
            
            debug('sendOpcMsg delayed execute');
...
nmCln.prototype.reportService = function(srv, fn, tmo){
    ...
    debug('reported service:'+JSON.stringify(srv));
...
nmCln.prototype.updateService = function(srv, fn, tmo){
    ...
    debug('updated service:'+JSON.stringify(srv));
...
nmCln.prototype.queryService = function(srv, fn, tmo){
    ...
    debug('query service:'+JSON.stringify(srv));
...
nmCln.prototype.allowAcl = function(address, port){
    ...
    if (local || (address === self.ipaddr)) {
        debug('always allow local network');
    } else {    
...
nmCln.prototype.checkStunable = function(vurle, fn){
    ...
    self.getvURLInfo(vurle, function(err, routing){
        ...
        if (err || !routing) {            
            ...
            // invalid vURL
            console.error(err+',setup STUN to peer failed, invalid vURL');
            ...
            self.getClntSdps(routing.dst.gid, function(err, sdps){
                if (!err) {
                    debug('SDPs answer:'+JSON.stringify(sdps));
                    ...
                    // return asymmetric NAT/FW count
                           if (self.natype===2 || peer.natype===2) {
                        // directed connection without nat/fw
                        self.checkStunCache[vurle] = 2;
                    } else if (self.natype===0 && peer.natype===0) {
                        // both of async nat/fw
                        self.checkStunCache[vurle] = 1;
                    } else if (self.natype===1 && peer.natype===1) {
                        // both of sync nat/fw
