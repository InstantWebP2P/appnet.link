...
nmCln.prototype._LSM_connectTurnAgent = function(fn, tmo){
    ...
    if (self.turnSrvs && self.turnSrvs.length) {
        ...
        self.turnagentConn.once('open', function(){            
            ...
            try {
                // !!!Prefer json format
                self.turnagentConn.send(JSON.stringify(tom), {binary: false, mask: true}, function(err){    
    ...
    if (self.conmode === SEP.SEP_MODE_CS) {
        ...
        self.bsrv.hpsrv.on('connection', function(client) {    
            ...
            try {
                ...
                // !!!Prefer json format
                client.send(JSON.stringify(coninfo), {binary: false, mask: true}, function(err){    
                    if (err) {
...
nmCln.prototype.offerSdp = function(fn, tmo){
    ...
    for (var k in self.conn) {
        ...
        try {
            // !!!Prefer json format
            self.conn[k].socket.send(JSON.stringify(opc_msg), {binary: false, mask: true}, function(err){
...
nmCln.prototype.sendOpcMsg = function(opc_msg, fn){
    ...
    if ((self.conn[con0] && self.conn[con0].socket) && 
        ...
        try {
            ...
            // !!!Prefer json format
            self.conn[con0].socket.send(JSON.stringify(opc_msg), {binary: false, mask: true}, function(err) {    
                if (err) {
