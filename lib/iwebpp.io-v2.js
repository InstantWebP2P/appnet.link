...
var SecureWebSocket = require('sws');
...
var nmCln = exports = module.exports = function(options, fn){
    ...
    var sslmode = (options.sslmode === undefined) ? 
    		SEP.SEP_SSL_AUTH_SRV_CLNT : options.sslmode; 
    // vpath or vhost 
...
nmCln.prototype._getValidIPAddr = function(proto, fn){
    ...
    function testConnection2NS(laddr, fn){
        ...
        var   nscon = new SecureWebSocket(wsproto+self.srvs[0].ip+':'+self.srvs[0].port+SEP.SEP_CTRLPATH_NS_V2,
        		{
...
nmCln.prototype._LSM_connectTurnAgent = function(fn, tmo){
    ...
    if (self.turnSrvs && self.turnSrvs.length) {
        ...
        if (Debug) console.log('turn agent connection:'+wsproto+self.turnSrvs[0].ip+':'+self.turnSrvs[0].agent+SEP.SEP_CTRLPATH_AS_V2);
        self.turnagentConn = new SecureWebSocket(wsproto+self.turnSrvs[0].ip+':'+self.turnSrvs[0].agent+SEP.SEP_CTRLPATH_AS_V2,
        		{
...
nmCln.prototype._LSM_setupSdpSession = function() {
    ...
    if (self.reconnect) {
    	conn = new SecureWebSocket(wsproto+self.srvs[0].ip+':'+self.srvs[0].port+SEP.SEP_CTRLPATH_NS_V2,
    			{
    ...
    conn.once('open', function(){
        ...
        for (var i = 1; i < self.srvs.length; i ++) {
            var connalt = new SecureWebSocket(wsproto+self.srvs[i].ip+':'+self.srvs[i].port+SEP.SEP_CTRLPATH_NS_V2,
            		{
            				httpp: true, hole: {port: self.port, addr: self.ipaddr},
            				// SSL related info
            				rejectUnauthorized: self.secmode ? true : false,
...
nmCln.prototype._LSM_setupBusinessServer = function(){
	...
	if (self.conmode === SEP.SEP_MODE_CS) {
	    ...
	    if (self.secmode) // https options: key/cert/ca/...
	        self.bsrv.srv = httpps.createServer(self.srvsslcerts).listen(self.port, self.ipaddr);
	    else
	    ...
	    self.bsrv.hpsrv.on('connection', function(client){	
	        ...
	        try {
                ...
                var clrtmo = setTimeout(function(){
...
nmCln.prototype.createConnection = function(to, fn){
    ...
    if ((to.sesn === SEP.SEP_SESN_STUN) &&
        ...
        conn = new SecureWebSocket(wsproto+peer.lip+':'+peer.lport+wstoken+SEP.SEP_CTRLPATH_BS_V2, opt);
        console.log('connect to '+wsproto+peer.lip+':'+peer.lport+wstoken+SEP.SEP_CTRLPATH_BS_V2);
    } else {
        // check vURL mode
        if (peer.vmode === vURL.URL_MODE_PATH) {
            // check session mode
            if (to.sesn === SEP.SEP_SESN_STUN) {
	            conn = new SecureWebSocket(wsproto+peer.ip+':'+peer.port+wstoken+SEP.SEP_CTRLPATH_BS_V2, opt);
	            console.log('connect to '+wsproto+peer.ip+':'+peer.port+wstoken+SEP.SEP_CTRLPATH_BS_V2);
            } else {
	            conn = new SecureWebSocket(wsproto+peer.ip+':'+peer.port+wstoken+peer.vpath+SEP.SEP_CTRLPATH_BS_V2, opt);
	            console.log('connect to '+wsproto+peer.ip+':'+peer.port+wstoken+peer.vpath+SEP.SEP_CTRLPATH_BS_V2);
            }
        } else {
            // check session mode
            if (to.sesn === SEP.SEP_SESN_STUN) {
                ...
                conn = new SecureWebSocket(wsproto+peer.vhost+peer.ip+':'+peer.port+wstoken+SEP.SEP_CTRLPATH_BS_V2, opt);
                console.log('connect to '+wsproto+peer.vhost+peer.ip+':'+peer.port+wstoken+SEP.SEP_CTRLPATH_BS_V2);
            }
        }
    }
	    ...
	    setTimeout(function(){
	        ...
	        if (to.isInitiator) {
		        ...
		        if ((self.oipaddr === peer.ip) || 
		            ...
		            conn = new SecureWebSocket(wsproto+peer.lip+':'+peer.lport+SEP.SEP_CTRLPATH_HS_V2, opt);
		        } else {
		            conn = new SecureWebSocket(wsproto+peer.ip+':'+peer.port+SEP.SEP_CTRLPATH_HS_V2, opt);
		        }
                    ...
                    self.offerStun({endpoint: peerinfo}, function(err, stun){
                        if (err || !stun) {
                            ...
                            if (self.trvsStunCache[vurle]) {
	                            ...
	                            setTimeout(function(){
	                                self.trvsStunCache[vurle] = null;    
	                            }, 600000); // force clean stun session after 10mins
                            }
                        }
                    });     
                } else {
