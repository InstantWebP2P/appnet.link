			                ...
			                self.updateNatype(function(err){
			                    if (!err) {
									...
									if (self.conmode === SEP.SEP_MODE_CS) {
									    ...
									    // TBD... optimizing backlog according to udt/udp buffer size,leave as 1024
									    if (self.secmode) // https options: key/cert/ca
									        self.bsrv.srv = httpps.createServer(self.secerts).listen(self.port, self.ipaddr, 1024);
									    else
									        self.bsrv.srv = httpp.createServer().listen(self.port, self.ipaddr, 1024);
...
// Web proxy service to proxy local http and websocket to peer over STUN
...
nmCln.prototype.webProxy = function(options){
    ...
    return {httpApp: appHttp, wsApp: wspxy};
};
