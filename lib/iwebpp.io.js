...
var nmCln = exports = module.exports = function(options, fn){
    ...
    self._getValidIPAddr(self.ipmode, function(err, addr) {
        if (!err && addr) {
            if (Debug) console.log('local ip address: '+addr);
    ...
    function onMessage(message, flags) {
        ...
        if ('number' === typeof data.opc) {
            ...
            case SEP.SEP_OPC_PUNCH_OFFER:
                if (Debug) console.log('got SEP_OPC_PUNCH_OFFER');
...
nmCln.prototype._LSM_setupBusinessServer = function(){
	...
	if (self.conmode === SEP.SEP_MODE_CS) {
        ...
        var bsrv_lisn_cb = function() {
            if (Debug) console.log('Business server started, Go ...');
            self.emit('go');
        };
...
nmCln.prototype.punchHole = function(to, fn){
    ...
    if (Debug) console.log('self.natype: '+self.natype+',peer.natype:'+peer.natype);
    ...
    if (self.natype===0 && peer.natype===0) {
	    ...
	    setTimeout(function(){
	        ...
	        if (to.isInitiator) {
                ...
                if (Debug) console.log('ws hole url: '+wshurl);
