...
nmCln.prototype._LSM_setupBusinessServer = function(){
	...
	if (self.conmode === SEP.SEP_MODE_CS) {
	    ...
	    // TBD... optimizing backlog according to udt/udp buffer size,leave as 1000
	    if (self.secmode) // https options: key/cert/ca
	        self.bsrv.srv = httpps.createServer(self.secerts).listen(self.port, self.ipaddr, 1000);
	    else
	        self.bsrv.srv = httpp.createServer().listen(self.port, self.ipaddr, 1000);
