...
var connectTurnAgent = function(self, fn, tmo){
    ...
    if (self.turnSrvs && self.turnSrvs.length) {
        ...
        self.turnagentConn = new WebSocket('ws://'+self.turnSrvs[0].ip+':'+self.turnSrvs[0].agent+SEP.SEP_CTRLPATH_AS, {hole: {port: self.port, addr: self.addr}});
...
var nmCln = exports = module.exports = function(options){
    ...
    self.oaddr   =          ''; // the out address seen by peer. if NAT exists, it should be NAT/gw address, not local address
    self.oport   =           0; // the out port seen by all peers. useless in case symmetry NAT/FW
            ...
            case SEP.SEP_OPC_SDP_ANSWER:
                ...
                if (data.answer.state === SEP.SEP_OPC_STATE_READY) {
                    ...
                    if (self.offans_sdp.length === self.srvs.length) {
                        ...
                        self.oport  = (symmetry ? 0 : self.offans_sdp[0].answer.sdp.clntPort); // useless in case symmetry NAT/FW
    ...
    // with 10s timeout.
    conn = new WebSocket('ws://'+self.srvs[0].ip+':'+self.srvs[0].port+SEP.SEP_CTRLPATH_NS, {hole: {addr: self.addr}});
    // initialize offer message count per client
    // every time, client send one offer message, increase it by one
    conn.offerMsgcnt = 0;
    ...
    conn.on('open', function(){
        ...
        for (i = 1; i < self.srvs.length; i ++) {
            conn = new WebSocket('ws://'+self.srvs[i].ip+':'+self.srvs[i].port+SEP.SEP_CTRLPATH_NS, {hole: {port: self.port, addr: self.addr}});
            // initialize offer message count per client
            // every time, client send one offer message, increase it by one
            conn.offerMsgcnt = 0;
                ...
                if (self.ocnt == self.srvs.length) {
					...
					if (conmode && (conmode.mode === SEP.SEP_MODE_CS)) {
						...
						case SEP.SEP_TYPE_SRV_HTTPP:
						    ...
						    self.bsrv.hpsrv.on('connection'+SEP.SEP_CTRLPATH_HS, function(client){	
                        ...
                        // 6.
						// emit connected event after start business server
