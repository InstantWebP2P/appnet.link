...
var nmCln = exports = module.exports = function(options){
        ...
        if ('number' === typeof data.opc) {
            ...
            case SEP.SEP_OPC_ALL_LOGIN_ANSWER:
                ...
                if (data.answer.state === SEP.SEP_OPC_STATE_READY) {
                    ...
                    // return error info
                    self.emit('alllogins'+data.evcbcnt, {err: 'ask user all logins failed'});
    ...
    // with 10s timeout.
    conn = new WebSocket('ws://'+self.srvs[0].ip+':'+self.srvs[0].port+SEP.SEP_CTRLPATH_NS, {httpp: true, hole: {addr: self.ipaddr}});
    // initialize offer message count per client
    // every time, client send one offer message, increase it by one
    conn.offerMsgcnt = 0;
						...
						if (self.conmode && (self.conmode.mode === SEP.SEP_MODE_CS)) {
							...
							case SEP.SEP_TYPE_SRV_HTTPP:
							    ...
							    self.bsrv.srv = httpp.createServer(self.conmode.srvapp || function(req, res){
							        ...
							        res.end('please hook application...');
							...
							default:
							    ...
							    // create httpp server
							    var httpsrv = httpp.createServer(function(req, res){
							        // default app logics
							        console.log('not httpp application...');
							        res.end('not httpp application...');
							    }).listen(self.port, self.ipaddr);
							    
							    // hook websocket business server on httpp server
	                            self.bsrv = {
	                                 srv: new WebSocketServer({httpp: true, server: httpsrv, path: self.vpath+SEP.SEP_CTRLPATH_BS}),
	                            ...
	                            ///////////////////////////////////////////////////////////////////////////////////////
	                            // hook websocket hole punch server on httpp server as well with different control path
	                            self.bsrv.hpsrv = new WebSocketServer({httpp: true, server: httpsrv, path: self.vpath+SEP.SEP_CTRLPATH_HS});
	                            self.bsrv.hpsrv.on('connection', function(client){	
	                                console.log('new ws connection: ' +
	                                             client._socket.remoteAddress+':'+client._socket.remotePort+' -> ' + 
	                                             client._socket.address().address+':'+client._socket.address().port);
