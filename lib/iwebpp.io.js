...
// -  ipmode: IPv6 or IPv4. 0: IPv4, 1: IPv6
var nmCln = exports = module.exports = function(options){
    ...
    // state machine: new->connecting->(connected/timeout/error)->(ready->reconnecting->(connected/timeout/error)->ready)->closing->closed
    self.state = 'new';
    ...
    self.ipaddr  =        null; // all coonection MUST bind on the same local interface
    
    // public/outter IP address info 
    self.gid     =   uuid.v4(); // GID of name-client
    ...
    var intfsaddr = self.ipmode ? addr6 : addr4;
    var intfskkok = 0;
			            ...
			            case SEP.SEP_OPC_SDP_ANSWER:
			                ...
			                if (data.answer.state === SEP.SEP_OPC_STATE_READY) {
			                    ...
			                    if (self.offans_sdp.length === self.srvs.length) {
			                        ...
			                        if (self.secmode) {
			                            // like https certification: {key: xxx, cert: xxx, ca: xxx, subdn: {'subdomain': {key: x, cert: x}}}
			                            self.secerts = self.offans_sdp[0].answer.secerts;
			                            
			                            if (self.secerts && self.secerts.subdn) {
		                            	    self.secerts.SNICallback = function(hostname) {
		                            	        var key, cert;
		                            	        		                            	        									        
										        if ((/[a-z]+\.[a-z]+\.vurl\./gi).test(hostname)) {
										            // like *.*.vurl.iwebpp.com 	
											        key  = self.secerts.subdn['x.x.vurl.'] && self.secerts.subdn['x.x.vurl.'].key;
											        cert = self.secerts.subdn['x.x.vurl.'] && self.secerts.subdn['x.x.vurl.'].cert;
										        } else if ((/[a-z]+\.vurl\./gi).test(hostname)) {
										            // like *.vurl.iwebpp.com
											        key  = self.secerts.subdn['x.vurl.'] && self.secerts.subdn['x.vurl.'].key;
											        cert = self.secerts.subdn['x.vurl.'] && self.secerts.subdn['x.vurl.'].cert;
										        } else {
											        key  = self.secerts.subdn['x.vurl.'] && self.secerts.subdn['x.vurl.'].key;
											        cert = self.secerts.subdn['x.vurl.'] && self.secerts.subdn['x.vurl.'].cert;     
										        }
										        ...
										        return crypto.createCredentials({
											        key:  key  || self.secerts.key,
											        cert: cert || self.secerts.cert
											    }).context;
										    };
			                            }
			    ...
			    conn.on('open', function(){
			        ...
			        for (i = 1; i < self.srvs.length; i ++) {
                        ...
                        connalt.on('close', function(){
                            ...
                            // trigger reconnect event
                            if (self.state === 'ready') {
                                self.emit('nsrecon', connalt.nmsrv);
                            }
                        });
...
nmCln.prototype.login = function(){

};

nmCln.prototype.logout = function(){

};

// get vURL info
// TBD... user authentication
var vurlgregex  = /(([0-9]|[a-f]){32}\.vurl\.)|(\/vurl\/([0-9]|[a-f]){32})/gi;

nmCln.prototype.getvURLInfo = function(vurl, fn, tmo){
    var self = this;
    
    // normalize vURL
    vurl = vurl.match(vurlgregex)[0];
    
    // vURL info cache with clear timer
    self.vurlInfoCache = self.vurlInfoCache || {};
    /*self.vurlInfoTimer = self.vurlInfoTimer || setInterval(function{
        self.vurlInfoCache = null;
    }, 30*60000);
    */
    
    if ((vurl in self.vurlInfoCache) && self.vurlInfoCache[vurl]) {
        return fn(null, self.vurlInfoCache[vurl]);
    }
    ...
    // send opc 
    self.sendOpcMsg(opc_msg, function(err){
        if (err && fn) return fn(err+',get vURL info failed'); 
    });    
};
...
nmCln.prototype.trvsSTUN = function(vurle, fn){
    ...
    // normalize vURL
    vurle = vurle.match(vurlgregex)[0];
    ...
    if ((vurle in self.trvsStunCache) && self.trvsStunCache[vurle]) {
        return fn(null, self.trvsStunCache[vurle]);
    }
    ...
    self.getvURLInfo(vurle, function(err, routing){
        ...
        if (err || !routing) {            
            ...
            self.getClntSdps(routing.dst.gid, function(err, sdps){
                if (!err) {
                    ///console.log('SDPs answer:'+JSON.stringify(sdps));
                    ...
                    self.offerStun({endpoint: peerinfo}, function(err, stun){
                        console.log('setup STUN to peer:'+JSON.stringify(peerinfo));
...
nmCln.prototype.trvsTURN = function(vurle, fn){
    ...
    if ((vurle in self.trvsTurnCache) && self.trvsTurnCache[vurle]) {
        return fn(null, self.trvsTurnCache[vurle]);
    }
            ...
            self.getClntSdps(routing.dst.gid, function(err, sdps){
                if (!err) {
                    ...
                    self.offerTurn({endpoint: peerinfo, sesn: SEP.SEP_SESN_TURN}, function(err, turn){
                        console.log('setup TURN to peer:'+JSON.stringify(peerinfo));
...
// Web proxy service to proxy local http and websocket to peer and forward proxy tunneling
...
// proxy mode: reverse or forward, default in reverse mode
// - options.forward == false means reverse proxy

nmCln.prototype.webProxy = function(options){
    var self = this;
    
    options = options || {};
    
    // proxy mode: reverse or forward
    options.forward = options.forward || false;

    // proxy cache
    self.webProxyCache = self.webProxyCache || {};
    
    // peer-proxy cache
    self.peerProxyCache = self.peerProxyCache || {};
    ...
    function httpxy(req, res, next) {
	    ...
	    if (vstrs = req.headers.host.match(vhostregex)) {
	        ...
	        // http forward proxy algorithm:
	        // - 1. find dedicated peer-proxy instance
	        // - 2. ask peer-proxy to proxy http request
	        // - 3. got proxed vURL and return redirection response
	        // - 4. then, going to https forward proxy 
	        if (options.forward) {
	            var hostname = req.headers.host.split(':')[0];
	            
	            if (!self.peerProxyCache[hostname]) {
	                // 1.1
	                // find peer-proxy instance for destination
	                self.peerProxyCache[hostname] = 'https://5ded793afcc1bab1220cb2064e154b53.vurl.iwebpp.com:51688/vtoken/8eb8094da1a62b29/';
	                
	                // temporary treat first peer-proxy as global agent
	                if (!self.peerProxyCache.gagent) {
	                    self.peerProxyCache.gagent = self.peerProxyCache[hostname];    
	                }
	            }
	            
	            // 1.2
	            // ask peer-proxy to proxy http request
	            var qsstr = 'vadmin/2/?url='+req.url;
	            
	            if (debug) console.log('qsstr:'+self.peerProxyCache[hostname]+qsstr); 				
	            var rreq = https.get(self.peerProxyCache[hostname]+qsstr, function(rres){
	                if (debug) console.log("Got response: " + rres.statusCode+','+JSON.stringify(rres.headers));
	                
	                // - 1.3
	                // got proxed vURL and return redirection response to vURL
	                res.writeHead(rres.statusCode, rres.headers);
		            res.end();
	            }).on('error', function(e) {
				    console.log("Got error: " + e.message);
				    next("Got error: " + e.message);
				});
	        } else {
		        // invalid vURL incase reverse proxy mode
	    ...
	    self.getvURLInfo(vurle, function(err, routing){
	        ...
	        if (err || !routing) {
		        ...
		        if (!self.webProxyCache[vurle]) {
	                ...
	                if ((self.oipaddr === routing.dst.ipaddr) ||
	                    (isLocalhost(self.oipaddr) && isLocalhost(routing.dst.ipaddr))) {
		        ...
		        self.trvsSTUN(vurle, function(err, stun){
		            if (err || !stun) {
			            // STUN not availabe
	                    res.writeHead(400);
    ...
    // ssl/https tunnel proxy based on CONNECT method
    function tunpxy(req, socket, head) {
	    var vurle, vstrs, urle = req.url;
	    ...
	    if (vstrs = urle.match(vhostregex)) {
	        ...
	        if (self.peerProxyCache[hostname]) {
	            // 2.1
	            // find peer-proxy 
                vurle = self.peerProxyCache[hostname].match(vurlgregex)[0];
            } else if (self.peerProxyCache.gagent) {
                // 2.2
                // default global peer-proxy
                vurle = self.peerProxyCache.gagent.match(vurlgregex)[0];
            } else {
                // 2.3
                // fallback to https forward prroxy
		        var urls    = URL.parse('http://'+req.url, true, true);
		        var srvip   = urls.hostname;
		        var srvport = urls.port || 443;
		            
	            if (debug) console.log('tunnel proxy for un-proxied site, connect to %s:%d', srvip, srvport);
				...
				srvSocket.on('error', function(e) {
				    console.log("tunnel proxy for un-proxied site, socket error: " + e);
				    socket.end();
				});
    ...
    // SOCKS proxy
    function sockspxy(socket, port, address, proxy_ready) {
	    ...
	    if (vstrs = urle.match(vhostregex)) {
            ...
            // TBD...
	        self.peerProxyCache[hostname] = 'https://xb4904cbe16529aa4ab8ac3e2676525bc.vurl.iwebpp.com:51688/vtoken/641f47450b7457a4/';
	        ...
	        if (self.peerProxyCache[hostname]) {
	            ...
	            if (debug) console.log('socks proxy for un-proxied site, connect to %s:%d', srvip, srvport);
	            var srvSocket = NET.connect(srvport, srvip, function() {
	                if (debug) console.log('socks proxy for un-proxied site, got connected!');   
				...
				srvSocket.on('error', function(e) {
				    console.log("socks proxy for un-proxied site, socket error: " + e);
				    socket.end();
				});
    ...
    return {httpApp: appHttp, wsApp: wspxy, tunnelApp: tunpxy, socksApp: sockspxy};
};
