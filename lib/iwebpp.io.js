...
var nmCln = exports = module.exports = function(options, fn){
    ...
    // (ready->reconnecting->connecting->(connected/timeout/error)->ready)->serving(business server started)
    // closing->closed
    // 
    self.state = 'new';
...
nmCln.prototype._LSM_setupBusinessServer = function(){
	...
	if (self.conmode === SEP.SEP_MODE_CS) {
        ...
        var bsrv_lisn_cb = function() {
            debug('Business server started ...');

            // emit business server started event as go
            self.emit('go');
        };
...
nmCln.prototype.punchHole = function(to, fn){
    ...
    // - a: one side is directed(Not in nat/fw), punching hole only from another side to the directed host
    // - b: both side are asymmetric, punching each other with a few pacekt
    // - d: only one side are asymmetric, punching from asymmetric only with a lot packet
    // - d: both side are symmetric, going to TURN session anyway
        
    // 0.
    // punch hole with at lease one directed host without NAT/FW
    if ((self.natype === 2) || (peer.natype === 2)) {
        fn('TBD...directed host'); return;
...
nmCln.prototype.offerStun = function(peer, fn, tmo){
    ...
    ///self.allowAcl(peer.endpoint.ip, 0);
