            ...
            case SEP.SEP_OPC_SDP_ANSWER:
                ...
                if (data.answer.state === SEP.SEP_OPC_STATE_READY) {
                    ...
                    if (self.offans_sdp.length === self.srvs.length) {
                        ...
                        if (self.secmode) {
                            // like https certification: {key: xxx, cert: xxx, ca: xxx}
                ...
                self.punchHole({
                 ...
                 function(err, coninfo){
                    ...
                    if (!err && coninfo) {
                        ...
                        data.answer.cinfo = coninfo; // pass my side of external ip/port info
                    } else {
...
nmCln.prototype._LSM_setupBusinessServer = function(){
	...
	if (self.conmode === SEP.SEP_MODE_CS) {
	    ...
	    self.bsrv.hpsrv.on('connection', function(client){	
	    	...
	    	// 1.2.2
	    	// send peer's connection info back about every 1s
	        try {
	            var coninfo = {peeraddr: client._socket.remoteAddress, peerport: client._socket.remotePort};
	            
                client.send(MSGPACK.encode(coninfo), {binary: true, mask: true}, function(err){
                    if (err) {
                        console.log(err+',sendOpcMsg failed');
                    }
                });
                
                var hpint = setInterval(function(){
	                client.send(MSGPACK.encode(coninfo), {binary: true, mask: true}, function(err){
	                    if (err) {
	                        console.log(err+',sendOpcMsg failed');
	                    }
...
nmCln.prototype.punchHole = function(to, fn){
    var self = this;
    var peer = to.endpoint;
    ///var opt = {httpp: true, hole: {port: self.port, addr: self.ipaddr, holeRange: {from: -8000, to: 8000}}};
    var opt = {httpp: true, hole: {port: self.port, addr: self.ipaddr}};
    var hole;
    var endinfo;
    ...
    if (!(self.natype || peer.natype)) {
	    ...
	    setTimeout(function(){
	        ...
	        if (to.isInitiator) {
		        ...
		        conn.once('message', function(data, flags) {
		            console.log('punch hole notification messaging in %s mode from %s',
		            (to.mode === SEP.SEP_MODE_PP) ? 'p2p' : 'c/s',
				    (to.isInitiator)? 'initiator' : 'peer');
				    ...
				    // 1.3.2
				    // maitian or close hole-punch connection
				    if (to.isInitiator) {
				        ...
				        // close connection launched by peer after 2s
					    setTimeout(function(){
					        conn.close()
					    }, 2000);
				    }
        ...
        if (self.natype) {
            ...
            // - if it's initiator, setup keep-hole connection to peer
		    ...
		    console.log('punch hole: '+self.ipaddr+':'+self.port+' -> '+endinfo.host+':'+endinfo.port+
		                '@range('+endinfo.localAddress.holeRange.from+'<->'+endinfo.localAddress.holeRange.to+')');
		    ...
		    setTimeout(function(){
		        ...
		        // 2.1.4
		        // pass my external ip/port info back
		        fn(null, {oipaddr: self.oipaddr, oport: self.oport});
...
nmCln.prototype.offerStun = function(peer, fn, tmo){
    ...
    if (self.natype || peer.natype) {
        fn('support only both are async NAT/FW');
        return self;
    }
        ...
        if (err || !routing) {            
            ...
            self.getClntSdps(routing.dst.gid, function(err, sdps){
                if (!err) {
				    ...
				    if (self.natype || peer.natype) {
				        self.checkStunCache[vurle] = false;
				    } else {
				        self.checkStunCache[vurle] = true;
				    }
				    fn(null, self.checkStunCache[vurle]);
				} else {
