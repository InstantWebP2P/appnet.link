                    ...
                    if (!err) {
						...
						if (self.conmode && (self.conmode.mode === SEP.SEP_MODE_CS)) {
							...
							case SEP.SEP_TYPE_SRV_HTTPP:
							    ...
							    self.bsrv.hpsrv = new WebSocketServer({server: self.bsrv.srv, path: self.vpath+SEP.SEP_CTRLPATH_HS});
							...
							default:
	                            ...
	                            self.bsrv = {
	                                 srv: new WebSocketServer({host: self.ipaddr, port: self.port, path: self.vpath+SEP.SEP_CTRLPATH_BS}),
