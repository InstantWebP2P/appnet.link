...
// generate GID
// URL like scheme://username:password@domain:port/path?query_string#fragment_id
var genUsrid = function(usrinfo){
    return ('usr://'+usrinfo.usrkey+'@'+usrinfo.domain);
};

var genClntid = function(clntinfo){
    return (clntinfo.proto+'://clnt:'+clntinfo.devkey+'@'+clntinfo.clntlocalIP+':'+clntinfo.clntlocalPort);
};

var genSrvid = function(srvinfo){
    return (srvinfo.proto+'://srv@'+srvinfo.srvpublicIP+':'+srvinfo.srvpublicPort);
};
            ...
            case SEP.SEP_OPC_SDP_ANSWER:
                ...
                if (data.answer.state === SEP.SEP_OPC_STATE_READY) {
                    ...
                    if (self.offans_sdp.length === self.srvs.length) {
                        ...
                        rsdp = {
                            symmetry: symmetry, // 0: cone NAT/FW, 1: symmetry NAT/FW
...
nmCln.prototype.offerStun = function(peer, fn, tmo){
    ...
    var mine = {
        natype: self.offans_sdp[0].answer.client.natype,
