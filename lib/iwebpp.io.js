...
nmCln.prototype.punchHole = function(to, fn){
    ...
    // 2.
    // create keep-hole connection after 3s
    setTimeout(function(){
        // clear interval timer after 3s, then close hole soon after 1s
        setTimeout(function(){
	        clearInterval(intl);
...
var vhostregex  = /([0-9]|[a-f]){32}\.vurl\./i;
var vpathregex  = /\/vurl\/([0-9]|[a-f]){32}/i;
var vtokenregex = /\/vtoken\/([0-9]|[a-f]){16}/i;
...
nmCln.prototype.webProxy = function(options){
		        ...
		        self.trvsSTUN(vurle, function(err, stun){
		            if (err || !stun) {
					    ...
					    if (stun.firstrun) {
				             ...
				             res.writeHead(301, {'location': (options.https ? 'https://' : 'http://')+req.headers.host+urle});
                             res.end();
				        } else {
    ...
    return {httpxy: httpxy, wspxy: wspxy};
};
