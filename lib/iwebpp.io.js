                ...
                self.updateNatype(function(err){
                    if (!err) {
		                ...
		                if (self.reconnect == 0) {
		                    ...
		                    // reconnect
		                    console.log('iWebPP.io reconnect ready');
...
nmCln.prototype.createConnection = function(to, fn){
    ...
    conn.on('close', function(){
	    ...
	    if (debug) console.log('new peer connection closed to '+JSON.stringify(to.endpoint));
    });
};
...
nmCln.prototype.punchHole = function(to, fn){
    ...
    setTimeout(function(){
        ...
        if (to.isInitiator) {
	        ...
	        conn.on('close', function(){
	            if (debug) console.log('punch hole closed in %s mode from %s',
	            (to.mode === SEP.SEP_MODE_PP) ? 'p2p' : 'c/s',
	            (to.isInitiator)? 'initiator' : 'peer');
	        });
