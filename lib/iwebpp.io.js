...
var eventEmitter = require('events').EventEmitter,
    ...
    udt = require('udt'), // for hole punch
...
var connectTurnAgent = function(self, fn, tmo){
    ...
    if (self.turnSrvs && self.turnSrvs.length) {
        ...
        self.turnagentConn = new WebSocket('ws://'+self.turnSrvs[0].ip+':'+self.turnSrvs[0].agent+SEP.SEP_CTRLPATH_AS, {httpp: true, hole: {port: self.port, addr: self.ipaddr}});
...
var nmCln = exports = module.exports = function(options){
    ...
    var secmode = (options.secmode === undefined) ? (options.https || false) : options.secmode; // secure mode: https, etc
    var   vmode = (options.vmode   === undefined) ? vURL.URL_MODE_PATH : options.vmode; // vpath or vhost 
    ...
    self.secerts =      ''; // secure certification in case https key/cert/ca, etc
    ...
    function testConnection2NS(laddr, fn){
        var nscon = new WebSocket('ws://'+self.srvs[0].ip+':'+self.srvs[0].port+SEP.SEP_CTRLPATH_NS, {httpp: true, hole: {addr: laddr}});
        var nstmo = setTimeout(function(){
            fn(0);
            console.log('connect to NS from '+laddr+' ... timeout');
        }, 1000); // 1s timeout
    ...
    for (var idx = 0; idx < addr4.length; idx ++) {
        ...
        testConnection2NS(addr4[idx], function(yes, addr){
            if (yes && !kkok4) {
			                    ...
			                    if (self.offans_sdp.length === self.srvs.length) {
			                        ...
			                        if (self.turnagentReady) {
			                            if (self.vmode === vURL.URL_MODE_PATH) {
			                                ...
			                                self.tvurl = 'http://'+self.srvinfo.turn[0].ip+':'+self.srvinfo.turn[0].proxy+self.vpath;
			                            } else {
			                                // vhost-based turn vURL
			                                // notes: vhost-based vURL MUST use domain name instead of ip address
			                                self.tvurl = 'http://'+self.vhost+self.srvinfo.turn[0].ip+':'+self.srvinfo.turn[0].proxy;
			                            }
			    ...
			    // with 10s timeout.
			    conn = new WebSocket('ws://'+self.srvs[0].ip+':'+self.srvs[0].port+SEP.SEP_CTRLPATH_NS,
			    ...
			    conn.on('open', function(){
			        ...
			        for (i = 1; i < self.srvs.length; i ++) {
			            conn = new WebSocket('ws://'+self.srvs[i].ip+':'+self.srvs[i].port+SEP.SEP_CTRLPATH_NS, {httpp: true, hole: {port: self.port, addr: self.ipaddr}});
			            // initialize offer message count per client
			            // every time, client send one offer message, increase it by one
			            conn.offerMsgcnt = 0;
			                ...
			                self.updateNatype(function(err){
			                    if (!err) {
									...
									if (self.conmode === SEP.SEP_MODE_CS) {
									    ...
									    // check vURL mode
									    if (self.vmode === vURL.URL_MODE_PATH) {
									        self.bsrv.hpsrv = new WebSocketServer({httpp: true, server: self.bsrv.srv, path: self.vpath+SEP.SEP_CTRLPATH_HS});
									    } else {
...
nmCln.prototype.createConnection = function(to, fn){
    ...
    // check session mode
    to.sesn = to.sesn || SEP.SEP_SESN_STUN;
    ...
    if (self.oipaddr === to.endpoint.ip) {
        ...
        if (to.endpoint.vmode === vURL.URL_MODE_PATH) {
            conn = new WebSocket('ws://'+to.endpoint.lip+':'+to.endpoint.lport+to.endpoint.vpath+SEP.SEP_CTRLPATH_BS, opt);
        } else {
            conn = new WebSocket('ws://'+to.endpoint.lip+':'+to.endpoint.lport+SEP.SEP_CTRLPATH_BS, opt);
        }
        console.log('LAN direct connection...');
    } else {
        // check vURL mode
        if (to.endpoint.vmode === vURL.URL_MODE_PATH) {
            conn = new WebSocket('ws://'+to.endpoint.ip+':'+to.endpoint.port+to.endpoint.vpath+SEP.SEP_CTRLPATH_BS, opt);
            console.log('connect to '+'ws://'+to.endpoint.ip+':'+to.endpoint.port+to.endpoint.vpath+SEP.SEP_CTRLPATH_BS);
        } else {
            // check session mode
            if (to.sesn == SEP.SEP_SESN_STUN) {
                conn = new WebSocket('ws://'+to.endpoint.ip+':'+to.endpoint.port+SEP.SEP_CTRLPATH_BS, opt);
                console.log('connect to '+'ws://'+to.endpoint.ip+':'+to.endpoint.port+SEP.SEP_CTRLPATH_BS);
            } else {
                conn = new WebSocket('ws://'+to.endpoint.vhost+to.endpoint.ip+':'+to.endpoint.port+SEP.SEP_CTRLPATH_BS, opt);
                console.log('connect to '+'ws://'+to.endpoint.vhost+to.endpoint.ip+':'+to.endpoint.port+SEP.SEP_CTRLPATH_BS);
            }
        }
    }
    ...
    setTimeout(function(){
        ...
        if (to.isInitiator) {
	        ...
	        if (self.oipaddr === to.endpoint.ip) {
	            // check vURL mode
                if (to.endpoint.vmode === vURL.URL_MODE_PATH) {
	                conn = new WebSocket('ws://'+to.endpoint.lip+':'+to.endpoint.lport+to.endpoint.vpath+SEP.SEP_CTRLPATH_HS, opt);
	            } else {
	            ...
	            // check vURL mode
                if (to.endpoint.vmode === vURL.URL_MODE_PATH) {
	                conn = new WebSocket('ws://'+to.endpoint.ip+':'+to.endpoint.port+to.endpoint.vpath+SEP.SEP_CTRLPATH_HS, opt);
	            } else {
