	...
	if (self.conmode === SEP.SEP_MODE_CS) {
	    ...
	    self.bsrv.hpsrv.on('connection', function(client) {	
	        ...
	        try {
    	        ...
    	        client.send(JSON.stringify(coninfo), {binary: false, mask: false}, function(err){	
                    if (err) {
...
nmCln.prototype.createConnection = function(to, fn){
    ...
    /*if ((to.sesn === SEP.SEP_SESN_STUN) &&
        ...
        conn = new WebSocket(wsproto+peer.lip+':'+peer.lport+wstoken+SEP.SEP_CTRLPATH_BS, opt);
    ...
    } else */
    {
        ...
        if (peer.vmode === vURL.URL_MODE_PATH) {
            // check session mode
            if (to.sesn === SEP.SEP_SESN_STUN) {
	            conn = new WebSocket(wsproto+peer.ip+':'+peer.port+wstoken+SEP.SEP_CTRLPATH_BS, opt);
	            console.log('connect to '+wsproto+peer.ip+':'+peer.port+wstoken+SEP.SEP_CTRLPATH_BS);
            } else {
	            conn = new WebSocket(wsproto+peer.ip+':'+peer.port+wstoken+peer.vpath+SEP.SEP_CTRLPATH_BS, opt);
            ...
            if (to.sesn === SEP.SEP_SESN_STUN) {
                ...
                conn = new WebSocket(wsproto+peer.vhost+peer.ip+':'+peer.port+wstoken+SEP.SEP_CTRLPATH_BS, opt);
	    ...
	    setTimeout(function(){
	        ...
	        if (to.isInitiator) {
		        ...
		        /*if ((self.oipaddr === peer.ip) || 
                    ...
                    wshurl = wsproto + peer.lip + ':' + peer.lport + SEP.SEP_CTRLPATH_HS;
                } else */
                {
    ...
    if (self.natype === 1 || peer.natype === 1) {
        ...
        if (self.natype === 1) {
		    ...
		    setTimeout(function(){
		        ...
		        /*if ((self.oipaddr === peer.ip) || 
	                (isLocalhost(self.oipaddr) && isLocalhost(peer.ip))) {
		            conn = new WebSocket(wsproto+peer.lip+':'+peer.lport+SEP.SEP_CTRLPATH_HS, opt);
                } else */
                {
		            conn = new WebSocket(wsproto+peer.ip+':'+peer.port+SEP.SEP_CTRLPATH_HS, opt);
		        }
...
nmCln.prototype.offerSdp = function(fn, tmo){
    ...
    for (var k in self.conn) {
        ...
        try {
            ...
            self.conn[k].socket.send(JSON.stringify(opc_msg), {binary: false, mask: false}, function(err){
...
nmCln.prototype.sendOpcMsg = function(opc_msg, fn){
	...
	if ((self.conn[con0] && self.conn[con0].socket) && 
		...
		try {
			...
			self.conn[con0].socket.send(JSON.stringify(opc_msg), {binary: false, mask: false}, function(err) {	
				if (err) {
