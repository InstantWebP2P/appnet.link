...
nmCln.prototype.createConnection = function(to, fn){
    ...
    if ((self.oipaddr === to.endpoint.ip) || 
        ...
        if (to.endpoint.vmode === vURL.URL_MODE_PATH) {
            conn = new WebSocket(wsproto+to.endpoint.lip+':'+to.endpoint.lport+wstoken+to.endpoint.vpath+SEP.SEP_CTRLPATH_BS, opt);
            console.log('connect to '+wsproto+to.endpoint.lip+':'+to.endpoint.lport+wstoken+to.endpoint.vpath+SEP.SEP_CTRLPATH_BS);
        } else {
