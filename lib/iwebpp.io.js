...
var nmCln = exports = module.exports = function(options){
    ...
    var secmode = (options.secmode === undefined) ? SEP.SEP_SEC_SSL  : options.secmode; // secure mode: https, etc
...
nmCln.prototype.createConnection = function(to, fn){
    ...
    if (self.oipaddr === to.endpoint.ip) {
        ...
        if (to.endpoint.vmode === vURL.URL_MODE_PATH) {
            conn = new WebSocket(wsproto+to.endpoint.lip+':'+to.endpoint.lport+to.endpoint.vpath+SEP.SEP_CTRLPATH_BS, opt);
            console.log('connect to '+wsproto+to.endpoint.lip+':'+to.endpoint.lport+to.endpoint.vpath+SEP.SEP_CTRLPATH_BS);
        } else {
            conn = new WebSocket(wsproto+to.endpoint.lip+':'+to.endpoint.lport+SEP.SEP_CTRLPATH_BS, opt);
            console.log('connect to '+wsproto+to.endpoint.lip+':'+to.endpoint.lport+SEP.SEP_CTRLPATH_BS);
        }
    } else {
        // check vURL mode
        if (to.endpoint.vmode === vURL.URL_MODE_PATH) {
            conn = new WebSocket(wsproto+to.endpoint.ip+':'+to.endpoint.port+to.endpoint.vpath+SEP.SEP_CTRLPATH_BS, opt);
            console.log('connect to '+wsproto+to.endpoint.ip+':'+to.endpoint.port+to.endpoint.vpath+SEP.SEP_CTRLPATH_BS);
        } else {
            // check session mode
            if (to.sesn == SEP.SEP_SESN_STUN) {
                conn = new WebSocket(wsproto+to.endpoint.ip+':'+to.endpoint.port+SEP.SEP_CTRLPATH_BS, opt);
                console.log('connect to '+wsproto+to.endpoint.ip+':'+to.endpoint.port+SEP.SEP_CTRLPATH_BS);
            } else {
                conn = new WebSocket(wsproto+to.endpoint.vhost+to.endpoint.ip+':'+to.endpoint.port+SEP.SEP_CTRLPATH_BS, opt);
                console.log('connect to '+wsproto+to.endpoint.vhost+to.endpoint.ip+':'+to.endpoint.port+SEP.SEP_CTRLPATH_BS);
            }
        }
    }
