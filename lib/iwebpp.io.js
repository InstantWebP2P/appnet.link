...
nmCln.prototype.webProxy = function(options){
	        ...
	        if (err || !routing) {
		        ...
		        self.trvsSTUN(vurle, function(err, stun){
		            if (err || !stun) {
					    ...
					    if (stun.firstrun) {
					        ///console.log((req.connection.encrypted ? 'https://' : 'http://')+req.headers.host+urle);
					        res.writeHead(301, {'location': (req.connection.encrypted ? 'https://' : 'http://')+req.headers.host+urle});
					        res.end();
				        } else {
	        ...
	        if (err || !routing) {
		        ...
		        self.trvsSTUN(vurle, function(err, stun){
		            if (err || !stun) {
		                ...
		                // 6.
					    // proxy target
					    // work-around first STUN setup hang by close socket 
					    if (stun.firstrun) {
					        ///console.log((options.https ? 'https://' : 'http://')+req.headers.host+urle);
					        socket.end('try it again');
				        } else {
    ...
    function tunpxy(req, socket, head) {
	    ...
	    if (vstrs = urle.match(vhostregex)) {
	        ...
	        // TBD ... discover peer to proxy it over STUN
