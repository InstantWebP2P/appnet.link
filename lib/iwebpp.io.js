...
var nmCln = exports = module.exports = function(options){
    ...
    // I.
    // SDP session setup function
    function setupSdpSession(self){
        // 0.1
        // initialize state
        self.ocnt = 0;
	    ...
	    conn.on('open', function(){
	        ...
	        for (i = 1; i < self.srvs.length; i ++) {
                ...
                connalt.on('close', function(){
                    console.log('name-server alternate client close');
                ...
                connalt.on('error', function(){
                    console.log('name-server alternate client error');
                });
	        }
	    });
    ...
    self.on('reconnect', function(event){
        console.log('Reconnect by '+event+', '+self.reconnect+' times');
	    ...
	    try {
	        ...
	        if (self.turnagentConn && self.turnagentConn.destroy) {
	            self.turnagentConn.destroy();   
	            self.turnagentConn = null;  
	        }
        } catch (e) {
            console.log('clear all sockets, ignore '+e);
        }
    ...
    self.once('ready', function(){
		...
		if (self.conmode === SEP.SEP_MODE_CS) {
		    ...
		    // TBD... optimizing backlog according to udt/udp buffer size,leave as 100
		    if (self.secmode) // https options: key/cert/ca
		        self.bsrv.srv = httpps.createServer(self.secerts).listen(self.port, self.ipaddr, 100);
		    else
		        self.bsrv.srv = httpp.createServer().listen(self.port, self.ipaddr, 100);
...
nmCln.prototype.offerSdp = function(fn, tmo){
    ...
    if (self.offans_sdp && self.offans_sdp.length) {
        for (var i=0; i < self.offans_sdp.length; i++)
            self.offans_sdp[i] = null;
