...
var connectTurnAgent = function(self, fn, tmo){
    ...
    if (self.turnSrvs && self.turnSrvs.length) {
        ...
        console.log('turn agent connection:'+wsproto+self.turnSrvs[0].ip+':'+self.turnSrvs[0].agent+SEP.SEP_CTRLPATH_AS);
...
var nmCln = exports = module.exports = function(options){
    ...
    var secmode = (options.secmode === undefined) ? SEP.SEP_SEC_SSL_ACL_HOST : options.secmode; // level-based ACL
    ...
    if (udt.isIP(options.srvinfo.turn[0].ip) || isLocalhost(options.srvinfo.turn[0].ip)) {
            ...
            if (yes && !kkok4) {
			                    ...
			                    if (self.offans_sdp.length === self.srvs.length) {
			                        ...
			                        if (self.turnagentReady) {
			                            ...
			                            if (self.vmode === vURL.URL_MODE_PATH) {
			                                ...
			                                self.tvurl = vurlproto+self.srvinfo.turn[0].ip+':'+self.srvinfo.turn[0].proxy+self.vpath;
			                            } else {
			                                // vhost-based turn vURL
			                                // notes: 
			                                // - vhost-based vURL MUST use domain name instead of ip address
			                                // - vhost-based vURL MUST Not use localhost
			                                self.tvurl = vurlproto+self.vhost+self.srvinfo.turn[0].ip+':'+self.srvinfo.turn[0].proxy;
			                            }
			    ...
			    conn.on('open', function(){
			        ...
			        for (i = 1; i < self.srvs.length; i ++) {
			            conn = new WebSocket(wsproto+self.srvs[i].ip+':'+self.srvs[i].port+SEP.SEP_CTRLPATH_NS, {httpp: true, hole: {port: self.port, addr: self.ipaddr}});
			            // initialize offer message count per client
			            // every time, client send one offer message, increase it by one
			            conn.offerMsgcnt = 0;
...
nmCln.prototype.createConnection = function(to, fn){
    ...
    var wstoken = (to.endpoint.secmode && to.sesn == SEP.SEP_SESN_TURN) ?
    ...
    if ((self.oipaddr === to.endpoint.ip) || 
        ...
        if (to.endpoint.vmode === vURL.URL_MODE_PATH) {
            conn = new WebSocket(wsproto+to.endpoint.lip+':'+to.endpoint.lport+to.endpoint.vpath+SEP.SEP_CTRLPATH_BS+wstoken, opt);
            console.log('connect to '+wsproto+to.endpoint.lip+':'+to.endpoint.lport+to.endpoint.vpath+SEP.SEP_CTRLPATH_BS+wstoken);
        } else {
            conn = new WebSocket(wsproto+to.endpoint.lip+':'+to.endpoint.lport+SEP.SEP_CTRLPATH_BS+wstoken, opt);
            console.log('connect to '+wsproto+to.endpoint.lip+':'+to.endpoint.lport+SEP.SEP_CTRLPATH_BS+wstoken);
        }
    } else {
        // check vURL mode
        if (to.endpoint.vmode === vURL.URL_MODE_PATH) {
            conn = new WebSocket(wsproto+to.endpoint.ip+':'+to.endpoint.port+to.endpoint.vpath+SEP.SEP_CTRLPATH_BS+wstoken, opt);
            console.log('connect to '+wsproto+to.endpoint.ip+':'+to.endpoint.port+to.endpoint.vpath+SEP.SEP_CTRLPATH_BS+wstoken);
        } else {
            // check session mode
            if (to.sesn == SEP.SEP_SESN_STUN) {
                conn = new WebSocket(wsproto+to.endpoint.ip+':'+to.endpoint.port+SEP.SEP_CTRLPATH_BS+wstoken, opt);
                console.log('connect to '+wsproto+to.endpoint.ip+':'+to.endpoint.port+SEP.SEP_CTRLPATH_BS+wstoken);
            } else {
                conn = new WebSocket(wsproto+to.endpoint.vhost+to.endpoint.ip+':'+to.endpoint.port+SEP.SEP_CTRLPATH_BS+wstoken, opt);
                console.log('connect to '+wsproto+to.endpoint.vhost+to.endpoint.ip+':'+to.endpoint.port+SEP.SEP_CTRLPATH_BS+wstoken);
            }
        }
    }
    ...
    setTimeout(function(){
        ...
        if (to.isInitiator) {
	        ...
	        if ((self.oipaddr === to.endpoint.ip) || 
	            ...
	            // check vURL mode
                if (to.endpoint.vmode === vURL.URL_MODE_PATH) {
	                conn = new WebSocket(wsproto+to.endpoint.lip+':'+to.endpoint.lport+to.endpoint.vpath+SEP.SEP_CTRLPATH_HS, opt);
	            } else {
	            ...
	            // check vURL mode
                if (to.endpoint.vmode === vURL.URL_MODE_PATH) {
	                conn = new WebSocket(wsproto+to.endpoint.ip+':'+to.endpoint.port+to.endpoint.vpath+SEP.SEP_CTRLPATH_HS, opt);
	            } else {
