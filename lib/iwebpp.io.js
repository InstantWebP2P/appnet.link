        ...
        if (peer.vmode === vURL.URL_MODE_PATH) {
            ...
            if (to.sesn === SEP.SEP_SESN_STUN) {
                ...
                if (6 === UDT.isIP(peer.ip)) {
                    //!!! work-around for local IPv6 SSL certs altname
                    if (isLocalhost(peer.ip)) {
                        wsburl = wsproto + 'ip6-localhost:' + peer.port + wstoken + SEP.SEP_CTRLPATH_BS;
                    } else {
	    ...
	    setTimeout(function() {
	        ...
	        if (to.isInitiator) {
                ...
                if (6 === UDT.isIP(peer.ip)) {
                    //!!! work-around for local IPv6 SSL certs altname
                    if (isLocalhost(peer.ip)) {
                        wshurl = wsproto+'ip6-localhost:'+peer.port+SEP.SEP_CTRLPATH_HS;
                    } else {
