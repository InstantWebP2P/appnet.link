...
var nmCln = exports = module.exports = function(options){
    ...
    for (var k in intfs) {
        for (var kk in intfs[k]) {
            if (((intfs[k])[kk].internal != true) && ('IPv4' === (intfs[k])[kk].family)) {
               // find foreign network interface
               addr4.push((intfs[k])[kk].address);
            }
            
            if (((intfs[k])[kk].internal != true) && ('IPv6' === (intfs[k])[kk].family)) {
               // find foreign network interface
			                ...
			                if (data.answer.state === SEP.SEP_OPC_STATE_READY) {
			                    ...
			                    if (self.offans_sdp.length === self.srvs.length) {
			                        ...
			                        if (self.turnagentReady) {
			                            self.vurl = 'http://'+self.srvinfo.turn[0].ip+':'+self.srvinfo.turn[0].proxy+self.vpath;
			                        }
...
nmCln.prototype.offerSdp = function(fn, tmo){
    ...
    opc_msg.offer = {
        ...
        conmode       : self.conmode || SEP.SEP_MODE_CS,
        secmode       : self.secmode || false, // security mode: https, etc
                                               // it it's https mode, name-server will send back https key/cert/ca
...
nmCln.prototype.offerStun = function(peer, fn, tmo){
    ...
    opc_msg.offer = {mine: mine, peer: peer, mode: self.conmode || SEP.SEP_MODE_CS};
